...
public class DrawerLayoutContainer extends FrameLayout {
  ...
  //  super(context);
  //
  //  minDrawerMargin = (int) (MIN_DRAWER_MARGIN * AndroidUtils.density + 0.5f);
  ...
  //        AndroidUtils.statusBarHeight = insets.getSystemWindowInsetTop();
  //        lastInsets = insets;
  //        drawerLayout.setWillNotDraw(
  //            insets.getSystemWindowInsetTop() <= 0 && getBackground() == null);
  ...
  public DrawerLayoutContainer(@NonNull Context context,
  ...
  public void openDrawer(boolean fast) {
    ...
    if (fast) {
      animatorSet.setDuration(Math.max((int) (200.0f / drawerLayout.getMeasuredWidth() * (
  ...
  public boolean onTouchEvent(MotionEvent ev) {
    ...
    if (allowOpenDrawer) {
      if (ev != null
        ...
        if (maybeStartTracking && !startedTracking && (dx > 0
            && dx / 3.0f > Math.abs(dy)
            && Math.abs(dx) >= AndroidUtils.getPixelsInCM(0.2f, true)
            || dx < 0 && Math.abs(dx) >= Math.abs(dy) && Math.abs(dx) >= AndroidUtils.getPixelsInCM(
            0.4f, true))) {
          ...
          || ev != null && ev.getPointerId(0) == startedTrackingPointerId && (ev.getAction()
        ...
        if (startedTracking
          ...
          boolean backAnimation =
              drawerPosition < drawerLayout.getMeasuredWidth() / 2.0f && (velX < 3500
                  || Math.abs(velX) < Math.abs(velY)) || velX < 0 && Math.abs(velX) >= 3500;
          if (!backAnimation) {
    ...
    for (int i = 0; i < childCount; i++) {
      ...
      if (BuildConfig.DEBUG) {
        if (drawerLayout != child) {
          child.layout(lp.leftMargin, lp.topMargin + getPaddingTop(),
              lp.leftMargin + child.getMeasuredWidth(),
              lp.topMargin + child.getMeasuredHeight() + getPaddingTop());
        } else {
          child.layout(-child.getMeasuredWidth(), lp.topMargin + getPaddingTop(), 0,
              lp.topMargin + child.getMeasuredHeight() + +getPaddingTop());
        }
      } else {
        try {
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    if (Build.VERSION.SDK_INT < 21) {
      inLayout = true;
      if (heightSize == AndroidUtils.displaySize.y + AndroidUtils.statusBarHeight) {
        if (getLayoutParams() instanceof MarginLayoutParams) {
          setPadding(0, AndroidUtils.statusBarHeight, 0, 0);
        }
    ...
    for (int i = 0; i < childCount; i++) {
      ...
      if (drawerLayout != child) {
        ...
        final int drawerWidthSpec =
            getChildMeasureSpec(widthMeasureSpec, minDrawerMargin + lp.leftMargin + lp.rightMargin,
                lp.width);
  ...
  protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
    ...
    if (scrimOpacity > 0 && drawingContent) {
      ...
      final float alpha = Math.max(0, Math.min(drawerPosition / AndroidUtils.dp(20), 1.0f));
      if (alpha != 0) {
        shadowLeft.setBounds((int) drawerPosition, child.getTop(),
            (int) drawerPosition + shadowLeft.getIntrinsicWidth(), child.getBottom());
